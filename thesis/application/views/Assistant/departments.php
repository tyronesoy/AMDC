...
<form name="form1" id="user_form" method="post" action="dashboard/addUser" enctype="multipart/forma-data">
                ...
                <?php if ($result->num_rows > 0) {
                  while($row = $result->fetch_assoc()) { ?>
                    <tr>
                      ...
                      <td>
                        <?php if($row['department_status'] == 'Active') { ?>
                        <div class="btn-group">
...
if(isset($_POST['btnEdit'])){
    ...
    if($result_update){
        ...
        mysqli_select_db($conn, "itproject");
        $notif = "insert into logs (log_date,log_description,user,module) VALUES ('".$datetoday."','".$new_departmentName." ".$new_departmentLocation." has been edited','".$this->session->userdata('fname')." ".$this->session->userdata('lname')."','".$this->session->userdata('type')."')";
...
if(isset($_POST['btnUpdate'])){
    $new_id=mysqli_real_escape_string($con,$_POST['txtid']);
    $new_departmentStatus=mysqli_real_escape_string($con,$_POST['txtdepartmentStatus']);
    ...
    $sqlupdate="UPDATE departments SET department_status='$new_departmentStatus' WHERE department_id='$new_id' ";
    $result_update=mysqli_query($con,$sqlupdate);
    ...
    if($result_update){
        ...
        $datetoday = date('Y\-m\-d\ H:i:s A');
        mysqli_select_db($conn, "itproject");
        $notif = "insert into logs (log_date,log_description,user,module) VALUES ('".$datetoday."','department status with id# ".$new_id." has been changed to ".$new_departmentStatus."','".$this->session->userdata('fname')." ".$this->session->userdata('lname')."','".$this->session->userdata('type')."')";
        $result = $conn->query($notif);
        echo '<script>window.location.href="departments"</script>';
    }
    else{
...
if(isset($_POST['depDelete'])){
    ...
    if($result_update){
        ...
        $datetoday = date('Y\-m\-d\ H:i:s A');
        mysqli_select_db($conn, "itproject");
        $notif = "insert into logs (log_date,log_description,user,module) VALUES ('".$datetoday."','department with id# ".$new_id." has been removed','".$this->session->userdata('fname')." ".$this->session->userdata('lname')."','".$this->session->userdata('type')."')";
        $result = $conn->query($notif);
        echo '<script>window.location.href="departments"</script>';
    }
    else{
