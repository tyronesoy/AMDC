          ...
          <div class="box">
            ...
            <!-- /.box-header -->
                   ...
                   <table id="example" class="table table-bordered table-striped">
                      <?php
                        $conn =mysqli_connect("localhost","root","", "itproject") or die('Error connecting to MySQL server.');
                        $sql = "SELECT * FROM inventory_order JOIN inventory_order_supplies USING(inventory_order_uniq_id) WHERE inventory_order_name ='".$this->session->userdata('fname').' '.$this->session->userdata('lname')."' AND inventory_order_status = 'Declined' GROUP BY inventory_order_id ";
                        $result = $conn->query($sql);    
                      ?>
                      <thead>
                          <tr>
                      ...
                      <tbody>
                        <?php if ($result->num_rows > 0) {
                          while($row = $result->fetch_assoc()) { ?>
                            ...
                            <?php if($row['inventory_order_status'] == 'Declined') { ?>
