...
if(isset($_POST['btnEdit'])){
    ...
    if($result_update){
        ...
        mysqli_select_db($conn, "itproject");
        $notif = "insert into logs (log_date,log_description,user,module) VALUES ('".$datetoday."','User account ".$new_username." has been edited with ".$new_username." user name,".$new_usercontact." user contact,".$new_email." email and ".$new_deptname." department','".$this->session->userdata('fname')." ".$this->session->userdata('lname')."','".$this->session->userdata('type')."')";
